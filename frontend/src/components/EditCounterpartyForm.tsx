...
import { Loader2, Mail, Phone, User, UserCog, ChevronDown, X, Search } from 'lucide-react';
import { Counterparty, Manager } from '@/types/counterparties';

interface EditCounterpartyFormProps {
  counterparty: Counterparty;
  isOpen: boolean;
  onClose: () => void;
  onSave: (id: number, data: Partial<Counterparty>) => Promise<void>;
  managers: Manager[];
}
...
export function EditCounterpartyForm({
  ...
  // Заповнюємо форму даними контрагента при відкритті
  useEffect(() => {
    if (isOpen && counterparty) {
      const initialManagerName = counterparty.responsible_manager_id 
        ? getManagerName(counterparty.responsible_manager_id)
        : '';
      
      setFormData({
        name: counterparty.name || '',
        counterparty_type: counterparty.counterparty_type || 'INDIVIDUAL',
        responsible_manager_id: counterparty.responsible_manager_id || null,
      ...
      setManagerSearch(prev => ({
        ...prev,
        searchQuery: initialManagerName,
        isDropdownOpen: false,
        filteredManagers: managers
      }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updateData: Partial<Counterparty> = {
      ...
      await onSave(counterparty.counterparty_id, updateData);
      onClose();
    } catch (error) {
      console.error('Помилка при збереженні контрагента:', error);
      setErrors({ submit: 'Помилка при збереженні. Спробуйте знову.' });
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="max-w-md mx-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Внесіть зміни в інформацію про контрагента
          </DialogDescription>
        </DialogHeader>
