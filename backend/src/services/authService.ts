...
export class AuthService {
  ...
  static async getCurrentUser(managerId: number) {
    const manager = await prisma.manager.findUnique({
  ...
  static async getSubordinateIds(managerId: number): Promise<number[]> {
    ...
    return subordinates.map((s: { manager_id: number }) => s.manager_id);
  }
}
