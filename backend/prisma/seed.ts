...
async function main() {
  ...
  // Create sale status types
  const saleStatuses = ['Оплачено', 'Відтермінована оплата', 'Не оплачено'];
  for (const name of saleStatuses) {
    await prisma.saleStatusType.upsert({
      where: { name },
      create: { name },
      update: {},
    });
  }
  console.log('🏷️  Created sale status types');
