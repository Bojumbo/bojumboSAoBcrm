...
const toSaleWithRelations = (sale) => {
    const productsTotal = sale.products.reduce((sum, item) => sum + item.product.price.toNumber() * item.quantity, 0);
    const servicesTotal = sale.services.reduce((sum, item) => sum + item.service.price.toNumber(), 0);
    return {
        ...sale,
...
export class SaleService {
    ...
    static async create(data) {
        const { products, services, ...raw } = data;
        ...
        const saleData = { ...raw, status: statusValue };
        // Remove non-persisted helper field if present
        if ('subproject_id' in saleData)
    ...
    static async update(id, data) {
        ...
        // Remove non-persisted helper field if present
        if ('subproject_id' in updateData)
            delete updateData.subproject_id;
