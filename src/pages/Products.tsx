  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="text-xl font-semibold">Товари</div>
        <GlassButton onClick={()=> setAddOpen(v=>!v)}>{addOpen ? 'Скасувати' : 'Додати товар'}</GlassButton>
      </div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              <GlassButton disabled={!name || price === '' || busy} onClick={async ()=>{
                try {
                  setBusy(true);
                  const created = await ProductsService.create({ name: name.trim(), price: Number(price) });
      ...
      <GlassCard className="p-4">
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            {filtered.map(it => (
              <div key={it.product_id} className="bg-white/5 border border-white/10 rounded-lg p-3 flex items-center gap-2">
                ...
                <GlassIconButton aria-label="Видалити" title="Видалити" onClick={async ()=>{
                  ...
                  try { await ProductsService.remove(it.product_id); setItems(prev => prev.filter(x=>x.product_id!==it.product_id)); }
